...
async def test_configured_postgres_has_identity_invariants() -> None:
    ...
    async with engine.connect() as connection:
        ...
        assert {"members", "space_memberships", "personal_api_keys", "audit_events"} <= tables
        global_name = await connection.scalar(text("SELECT name FROM workspaces WHERE id = 'global'"))
