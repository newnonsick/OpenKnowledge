...
GRANT SELECT ON alembic_version, api_key_quota_policies, api_key_scopes, api_key_space_grants, api_key_budget_usage, audit_events, compatibility_principals, document_chunks, document_files, document_revision_chunks, document_revisions, documents, embedding_generations, idempotency_records, ingestion_jobs, knowledge_items, knowledge_revisions, login_throttle_buckets, members, mfa_factors, mfa_recovery_codes, password_credentials, pending_ai_actions, personal_api_keys, provenance_links, retrieval_units, runtime_setting_revisions, session_credentials, session_families, source_connector_files, source_connectors, space_memberships, webhook_deliveries, webhook_subscriptions, workspaces TO gateway_runtime;
GRANT INSERT ON api_key_quota_policies, api_key_scopes, api_key_space_grants, api_key_budget_usage, audit_events, document_chunks, document_files, document_revisions, documents, idempotency_records, ingestion_jobs, job_outbox, knowledge_items, knowledge_revisions, login_throttle_buckets, members, mfa_factors, mfa_recovery_codes, password_credentials, pending_ai_actions, personal_api_keys, provenance_links, retrieval_units, runtime_setting_revisions, session_credentials, session_families, source_connector_files, source_connectors, space_memberships, webhook_deliveries, webhook_subscriptions, workspaces TO gateway_runtime;
GRANT UPDATE ON api_key_budget_usage, api_key_quota_policies, document_chunks, document_files, idempotency_records, knowledge_items, login_throttle_buckets, members, mfa_factors, mfa_recovery_codes, password_credentials, personal_api_keys, session_credentials, session_families, source_connector_files, source_connectors, webhook_deliveries, webhook_subscriptions TO gateway_runtime;
GRANT DELETE ON api_key_quota_policies, api_key_scopes, api_key_space_grants, document_chunks, document_files, knowledge_items, source_connector_files, source_connectors, space_memberships TO gateway_runtime;
...
GRANT EXECUTE ON FUNCTION gateway_actor_active(), gateway_has_space_role(text, text[]), gateway_is_initial_space_owner(text, uuid), gateway_can_change_membership(text, uuid, text), gateway_actor_super_admin(), gateway_key_space_grants(uuid), gateway_key_may_use_space(uuid, text) TO gateway_runtime;
GRANT SELECT ON document_revision_chunks, document_revisions, documents, embedding_generations, ingestion_jobs, job_outbox, knowledge_items, knowledge_revisions, members, operational_alerts, retrieval_units, space_memberships, webhook_deliveries, webhook_subscriptions, workspaces TO gateway_worker;
...
GRANT UPDATE (embedding) ON knowledge_revisions TO gateway_worker;
GRANT UPDATE (active, deactivated_at) ON retrieval_units TO gateway_worker;
GRANT UPDATE (state, attempt_count, last_error_code, last_status_code, lease_owner, lease_expires_at, claim_token, available_at, delivered_at, updated_at) ON webhook_deliveries TO gateway_worker;
DO $$
DECLARE
