...
class SourceConnectorModel(Base):
    __tablename__ = "source_connectors"

    id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, default=uuid4)
    space_id: Mapped[str] = mapped_column(String(64), ForeignKey("workspaces.id", ondelete="CASCADE"), nullable=False)
    kind: Mapped[str] = mapped_column(String(16), default="git", server_default=text("'git'"), nullable=False)
    ...
    status: Mapped[str] = mapped_column(String(24), default="active", server_default=text("'active'"), nullable=False)
    created_by_member_id: Mapped[UUID | None] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("members.id", ondelete="SET NULL"))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)

    __table_args__ = (
        CheckConstraint("kind = 'git'", name="ck_source_connectors_kind"),
        CheckConstraint("status IN ('active','deleted')", name="ck_source_connectors_status"),
        UniqueConstraint("id", "space_id", name="uq_source_connectors_id_space"),
        UniqueConstraint("space_id", "repo_root", "branch", name="uq_source_connectors_space_repo_branch"),
        Index("ix_source_connectors_space", "space_id"),
    )


class SourceConnectorFileModel(Base):
    __tablename__ = "source_connector_files"
    ...
    document_id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), nullable=False)
    last_synced_checksum: Mapped[str | None] = mapped_column(String(64))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    ...
    __table_args__ = (
        ...
        UniqueConstraint("connector_id", "repo_path", name="uq_source_connector_files_connector_path"),
        UniqueConstraint("connector_id", "document_id", name="uq_source_connector_files_connector_document"),
        Index("ix_source_connector_files_connector", "connector_id"),
    )
